...
public:
    ...
    // Returns flat index: z * (width * height) + y * width + x
    // No bounds check - caller must ensure coordinates are valid
    int getFlatIndex(const int x, const int y, const int z) const
    {
        return z * (width * height) + y * width + x;
    }

    // Access raw data buffer (const reference, avoid copy)
